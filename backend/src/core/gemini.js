...
async function generateWithFallback({
  ...
  for (const model of MODEL_FALLBACK_CHAIN) {
    ...
    for (let tentativa = 1; tentativa <= 2; tentativa++) {
      try {
        console.log(
          `[Lumen IA] Solicitando com modelo: ${model} (tentativa ${tentativa})...`,
        );
        ...
        if (tentativa === 1) {
          console.log(
            `[Lumen IA] Aguardando 1.5s antes de retentar ${model}...`,
          );
          await sleep(1500);
        }
      }
    ...
    console.log(
      `[Lumen IA] Modelo ${model} segue indisponível. Alternando para o próximo modelo reserva...`,
    );
  }
