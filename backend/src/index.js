...
async function runMigrations() {
    try {
        ...
        const checkOficiais = await db.query("SELECT COUNT(*) FROM questoes WHERE origem != 'IA'");
        if (parseInt(checkOficiais.rows[0].count, 10) === 0) {
            ...
            for (const q of questoesIniciais) {
                await db.query(
                    ...
                    [q.materia, q.topico, q.ano_escolar_alvo, q.tipo_questao, q.pergunta, q.alternativas, q.gabarito, q.origem]
                );
            }
